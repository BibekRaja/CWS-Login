...
final class CWSL_Plugin
{
	...
	private function cwsl_load_front_controller()
	{
		...
		if (!defined('WP_USE_THEMES')) {
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound -- Core expects this constant before loading templates.
	...
	/**
	 * Gets the raw request URI for routing checks only.
	 *
	 * @return string
	 */
	private function cwsl_get_request_uri()
	{
		...
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- We only parse this for routing and never print it.
	...
	public function cwsl_plugins_loaded_routing()
	{
		...
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Public URL check for the login slug, not a state-changing form.
	...
	public function cwsl_wp_loaded()
	{
		...
		// phpcs:disable WordPress.Security.NonceVerification -- This mirrors core wp-login request handling on the front end.
		$request_uri = $this->cwsl_get_request_uri();
		$parsed = wp_parse_url($request_uri);
		...
		if ('wp-login.php' === $pagenow) {
			/*
			 * wp-login.php reads these globals.
			 * Since we require the file from here, they may not be set yet,
			 * so we initialize them first to avoid notices and login form issues.
			 */
	...
	{
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Public confirm link values are validated before any redirect happens.
		if (
			empty($_GET['action'])
		...
		$request_id = (int) $_GET['request_id']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Checked right below with wp_validate_user_request_key().
		$key = sanitize_text_field(wp_unslash($_GET['confirm_key'])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Checked right below with wp_validate_user_request_key().
	...
	private function cwsl_rewrite_wp_login_in_url($url, $scheme = null)
	{
		...
		// phpcs:disable WordPress.Security.NonceVerification -- URL filtering only; follows core behavior for these request params.
	...
	/**
	 * Keeps privacy export confirm links aligned with core email placeholders.
	 *
	...
	public function cwsl_maybe_redirect_old_settings()
	{
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only redirect check; no data is changed here.
