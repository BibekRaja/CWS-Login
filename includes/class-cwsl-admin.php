...
final class CWSL_Admin
{
	...
	public function cwsl_sanitize_login_slug($value)
	{
		...
		if (isset($_POST[CWSL_Plugin::OPTION_REDIRECT_SLUG])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Submitted via options.php where nonce/caps are already checked.
			$other = sanitize_title_with_dashes(wp_unslash($_POST[CWSL_Plugin::OPTION_REDIRECT_SLUG])); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Same settings request as above.
			if ($slug === $other) {
				add_settings_error(
					CWSL_Plugin::OPTION_LOGIN_SLUG,
	...
	public function cwsl_sanitize_redirect_slug($value)
	{
		...
		if (isset($_POST[CWSL_Plugin::OPTION_LOGIN_SLUG])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Submitted via options.php where nonce/caps are already checked.
			$other = sanitize_title_with_dashes(wp_unslash($_POST[CWSL_Plugin::OPTION_LOGIN_SLUG])); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Same settings request as above.
			if ($slug === $other) {
				add_settings_error(
					CWSL_Plugin::OPTION_REDIRECT_SLUG,
	...
	public function cwsl_settings_saved_notice()
	{
		...
		// phpcs:disable WordPress.Security.NonceVerification.Recommended -- Read-only notice check on query args.
				...
				<table class="form-table" role="presentation">
					<tbody>
						<tr>
							...
							<td>
								<?php if ($pretty): ?>
									...
									<input name="<?php echo esc_attr(CWSL_Plugin::OPTION_LOGIN_SLUG); ?>" id="cwsl_login_slug"
										type="text" class="regular-text" value="<?php echo esc_attr($login_slug); ?>"
										autocomplete="off" />
						...
						<tr>
							<th scope="row">
								<label
									for="cwsl_redirect_slug"><?php esc_html_e('Redirect path (blocked access)', 'cws-login'); ?></label>
							</th>
							<td>
								<?php if ($pretty): ?>
									<code class="cwsl-url-prefix"><?php echo esc_html($home); ?></code>
									<input name="<?php echo esc_attr(CWSL_Plugin::OPTION_REDIRECT_SLUG); ?>"
										id="cwsl_redirect_slug" type="text" class="regular-text"
										value="<?php echo esc_attr($redirect); ?>" autocomplete="off" />
									<code><?php echo $core->cwsl_trailing_slash_preference() ? ' /' : ''; ?></code>
								<?php else: ?>
									<code class="cwsl-url-prefix"><?php echo esc_html($home); ?>?</code>
									<input name="<?php echo esc_attr(CWSL_Plugin::OPTION_REDIRECT_SLUG); ?>"
						...
						<tr>
							...
							<td>
								<input type="hidden" name="<?php echo esc_attr(CWSL_Plugin::OPTION_LOGO_ID); ?>"
									id="cwsl_logo_attachment_id" value="<?php echo esc_attr((string) $logo_id); ?>" />
								<div id="cwsl-logo-preview" class="cwsl-logo-preview"
								...
								<p>
									<button type="button" class="button"
										id="cwsl-select-logo"><?php esc_html_e('Select image', 'cws-login'); ?></button>
